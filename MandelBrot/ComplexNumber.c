...
{
	ComplexNumber * okay = (ComplexNumber*)malloc(sizeof(ComplexNumber));
	okay->real = real_component;
        okay->imaginary = imaginary_component;
	return okay;
}
...
ComplexNumber* ComplexProduct(ComplexNumber* a, ComplexNumber* b)
{
	...
	double aIma  = Im(a);
	double bIma  = Im(b);
	return newComplexNumber(aReal * bReal - aIma * bIma, aReal * bIma + bIma * aReal);
}
...
ComplexNumber* ComplexSum(ComplexNumber* a, ComplexNumber* b)
{
	...
	double aIma  = Im(a);
	double bIma  = Im(b);
	return newComplexNumber(aReal + bReal, aIma+ bIma);
}
...
double ComplexAbs(ComplexNumber* a)
{
   	double aReal = Re(a);
	double aIma  = Im(a);
	return sqrt(aReal*aReal + aIma*aIma);
}
