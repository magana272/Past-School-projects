...
public class MovingRotor extends Rotor {
    ...
    public boolean atNotch() {
        int check = setting();
        for (int i = 0; i <= _notches.length() - 1; i++) {
            if (alphabet().toInt(_notches.charAt(i)) == check) {
                return true;
            }
        }
        return false;}
    ...
    // Use this special tag when updating the behavior of a method this class inherits from Rotor
    @Override
    public void advance() {
