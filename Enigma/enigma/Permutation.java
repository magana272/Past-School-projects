...
public class Permutation {
     ...
     *  Whitespace is ignored. */
      public Permutation(String cycles, Alphabet alphabet) {
        _alphabet = alphabet;
        _cycle = cycles;
        // F - Assign any additional instance variables.
    }

    /** Return the value of P modulo the size of this permutation. */
    public final int wrap(int p) {
        int r = p % size();
        if (r < 0) {
            r += size();
        }
    ...
    public int permute(int p) {
        ...
        char charmine = permute(letter);
        return _alphabet.toInt(charmine);
    ...
    public int invert(int c) {
        char track = _alphabet.toChar(c);
        char track2 = invert(track);
        return _alphabet.toInt(track2);
    }
    ...
    public char permute(char p) {
        ...
        char result= '.';
        int intemperate = 0;
        if (_cycle.indexOf(p)<0){
            return p;
        }
        while (result == '.') {
            if (_cycle.charAt(intemperate) == p){
                if (_cycle.charAt(intemperate+1) == ')') {
                    while (_cycle.charAt(intemperate) != '('){
                        intemperate -= 1;
                    }
                    result = _cycle.charAt(intemperate+1);
                    return result;
                }
                result = _cycle.charAt(intemperate + 1);
            }
    ...
    public char invert(char c){
        ...
        int indexers = 0;
        char chariness = '.';
        if (_cycle.indexOf(c)<0){
            return c; }
        while (chariness == '.') {
            if (_cycle.charAt(indexers) == c) {
                if (_cycle.charAt(indexers - 1) == '(' ){
                    while (_cycle.charAt(indexers) != ')') {
                        indexers += 1;
                    }
                }
                chariness = _cycle.charAt(indexers-1);
            }
            indexers ++;
        }
        return chariness;
