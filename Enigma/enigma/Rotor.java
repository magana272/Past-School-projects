...
public class Rotor {
    ...
    public void set(char cposn) {
        int input = alphabet().toInt(cposn);
        set(input);
    ...
    public int convertForward(int p) {
        int _in =_permutation.wrap(p + _setting);
        int per =_permutation.permute(_in) ;
        int out =_permutation.wrap(per -_setting);
        return out;
    ...
    public int convertBackward(int c) {
        int _in =_permutation.wrap(c + _setting);
        int per =_permutation.invert(_in) ;
        int out =_permutation.wrap(per-_setting);
        return out;
