...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        arg = rest.map(lambda oper :scheme_eval(oper, env))
        return scheme_apply(function, arg, env)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            ...
            while formals != nil:
                frames.bindings[formals.first] = vals.first
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    valone, valtwo = nil , nil
    while bindings :
        value = bindings.first
        if check_form(value, 1, 2):
            raise SchemeError
        bindings = bindings.second
        valone, valtwo  = Pair(value.first, valone), Pair(scheme_eval(value.second.first,env), valtwo )
        check_formals(valone)
    return env.make_child_frame(valone,valtwo)
