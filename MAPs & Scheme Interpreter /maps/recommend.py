...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        restaurant= group_by_centroid(restaurants,centroids)
        centroids = [find_centroid(res) for res in restaurant]
        # END Question 6
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    list_of_r_squared = [find_predictor(user, reviewed, fea)[0] for fea in feature_fns]
    return max(list_of_r_squared, key = lambda x: find_predictor(user, reviewed,x)[1])
    #return max(list_of_r_squared , key = lambda x: x[0])
    ##max(list_of_r_squared, key = lambda x :x[1])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for res in restaurants:
        if res in reviewed:
            dict[restaurant_name(res)]= user_rating(user,restaurant_name(res))
        else:
            dict[restaurant_name(res)] = predictor(res)
    return dict
